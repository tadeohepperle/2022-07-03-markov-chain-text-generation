_937‍.x([["tokenizeText",()=>tokenizeText],["buildMarkovModel",()=>buildMarkovModel],["randomKey",()=>randomKey],["generateTextFromMarkovModels",()=>generateTextFromMarkovModels],["arrayOfTokensToText",()=>arrayOfTokensToText],["buildMarkovModels",()=>buildMarkovModels],["generateMarkovTextFromText",()=>generateMarkovTextFromText],["scaleMarkovModelToTextLength",()=>scaleMarkovModelToTextLength],["joinMultiOrderModels",()=>joinMultiOrderModels],["generateSentenceFromMarkovModels",()=>generateSentenceFromMarkovModels]]);function tokenizeText(text) {
  const replacements = {
    ".": " . ",
    "?": " ? ",
    "!": " ! ",
  ...
  return text.split(/\s+/).filter((e) => e); // filter for removing empty string
       ...
       function buildMarkovModel(tokens, order) {
  ...
  for (let k of Object.keys(dict)) {
    ...
    if (r < probAtK + acc) {
      ...
      acc = acc + probAtK;
       ...
       function* generateTextFromMarkovModels(
  models,
  length = 100,
  startingWords = []
) {
  let order = models.length - 1;
  let words = [...startingWords];
  let modelOrder = words.length;
  for (let i = 0; i < length; i++) {
    modelOrder = Math.min(modelOrder, order);
    let key = words.slice(words.length - modelOrder, words.length).join("|");
    let probabilites = models[modelOrder][key];
    if (probabilites) {
      let nextWord = randomKey(probabilites);
      words.push(nextWord);
      yield nextWord;
      modelOrder += 1;
    } else {
  ...
  //return arrayOfTokensToText(words);
}

       function arrayOfTokensToText(tokens) {
  let text = "";
  ...
  return text;
       ...
       function buildMarkovModels(text, upToOrder = 2) {
  ...
  return Array(upToOrder)
    ...
    .map((_, i) => buildMarkovModel(tokens, i));
       ...
       function generateMarkovTextFromText(text) {
  let tokens = tokenizeText(text);
  ...
  return generateTextFromMarkovModels(models, 100);
       ...
       function scaleMarkovModelToTextLength(model, textLength) {
...
// modelsarray is a [k][order]model array
       function joinMultiOrderModels(modelsArray, order) {
  let totalModels = [];
  ...
  return totalModels;
       ...
       function generateSentenceFromMarkovModels(models, maxLength) {
  let startWord = randomKey(models[0][""]);
  let words = [startWord];
  while (true) {
    let generator = generateTextFromMarkovModels(models, 1000, words);
    let nextToken = generator.next().value;
    words.push(nextToken);
    if ([".", "?", "!"].find((e) => e == nextToken)) {
      break;
    }
  }
